...
class DeviceManager {
    constructor(config, eventEmitter) {
        ...
        this.logger = new logger_1.Logger('DeviceManager');
        this.config = config;
    ...
    getDefaultProperties(deviceType) {
        switch (deviceType) {
            case 'smart_light':
    ...
    controlLight(deviceState, command) {
    ...
    controlThermostat(deviceState, command) {
    ...
    controlSpeaker(deviceState, command) {
        const { action } = command;
        switch (action) {
            case 'start':
            case 'play':
    ...
    controlGenericDevice(deviceState, command) {
        const { action } = command;
        switch (action) {
            case 'turn_on':
                deviceState.properties.power = true;
