...
exports.LanguageProcessing = void 0;
...
class LanguageProcessing {
    constructor(config) {
        ...
        const logLevel = logger_2.LogLevel[logLevelString] || logger_2.LogLevel.INFO;
        this.logger = new logger_1.Logger('LanguageProcessing', logLevel);
    ...
    calculatePatternConfidence(match, command) {
    ...
    normalizeEntityValue(value, entityType) {
    ...
    determineAction(intent, entities, command) {
        switch (intent.name) {
            case 'device_control':
    ...
    determineDeviceAction(entities, command) {
        ...
        if (/turn\s+on|switch\s+on/i.test(command))
            return 'turn_on';
        if (/turn\s+off|switch\s+off/i.test(command))
            return 'turn_off';
        if (/dim/i.test(command))
            return 'dim';
        if (/brighten/i.test(command))
            return 'brighten';
        if (/start|play/i.test(command))
            return 'start';
        if (/stop|pause/i.test(command))
            return 'stop';
        if (/set.*to/i.test(command))
            return 'set_value';
        return 'control';
    }
    determineSystemAction(command) {
        if (/stop|pause|halt/i.test(command))
            return 'stop';
        if (/resume|continue/i.test(command))
            return 'resume';
        if (/restart|reboot/i.test(command))
            return 'restart';
        if (/help|assist/i.test(command))
            return 'help';
        if (/volume\s+up/i.test(command))
            return 'volume_up';
        if (/volume\s+down/i.test(command))
            return 'volume_down';
        if (/repeat/i.test(command))
            return 'repeat';
    ...
    addCustomEntity(entityType, pattern) {
...
//# sourceMappingURL=languageProcessing.js.map